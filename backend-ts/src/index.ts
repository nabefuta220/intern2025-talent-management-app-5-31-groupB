...
app.get("/api/employees", async (req: Request, res: Response) => {
  ...
  const filterAffiliation = req.query.filterAffiliation ?? "";
  ...
  if (Array.isArray(filterAffiliation)) {
    res.status(400).send();
    return;
  }
  if (typeof filterAffiliation !== "string") {
    res.status(400).send();
    return;
  }
  ...
  if (typeof filterPosition !== "string") {
    res.status(400).send();
    return;
  }
  try {
